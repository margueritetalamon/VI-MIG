...
from src.kl_computation import KL_computation
...
def run_xp(xp, pi_mean, pi_cov, mu_init, epsilon_init, folder_name, hyperparam = {}):
    ...
    if xp == "mirror":
        ...
        kls = KL_computation(M, E, pi_mean, pi_cov, M = 100)
    ...
    if xp == "optim_wo_eps":
        ...
        kls = KL_computation(M, None, pi_mean, pi_cov, M = 100)
...
def main(args):
    ...
    N_target = 30
    s = 5 * math.sqrt(d)
    pi_mean, pi_cov = target_gmm_4modes(4, d)
    ...
    for N_mixture in n_values:
        ...
        for nxp in range(nb_xps):
            ...
            epsilon_init = torch.ones(N_mixture)*math.sqrt(d)
            print(epsilon_init)
